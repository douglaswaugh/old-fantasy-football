    ...
    public class PoissonMatrix : ProbabilityCalculator
    {
        ...
        public double[][] GetPrediction(ExpectedGoals expectedGoals)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    var homeGoalsPoisson = new Poisson(Convert.ToDouble(expectedGoals.ExpectedHomeGoals));
                    var awayGoalsPoisson = new Poisson(Convert.ToDouble(expectedGoals.ExpectedAwayGoals));
